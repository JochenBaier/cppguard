...
{
bool update_lock_history_after_lock(const void* mu, const uint64_t& id, SynchLocksHeld& held_locks, const runtime_options_t& p_runtime_options)
{
  ...
  if(held_locks.locks[i].count==1) {
    ...
    held_locks.locks[n-1].mu=nullptr;
    held_locks.locks[n-1].count=0;
  } else {
