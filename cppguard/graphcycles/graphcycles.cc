...
bool GraphCycles::CheckInvariants() const
{
  ...
  for(uint32_t x=0; x<r->nodes_.size(); x++) {
    ...
    if(ptr!=nullptr&&static_cast<uint32_t>(r->ptrmap_.Find(ptr))!=x) {
      printf("CppGuard: Internal error: Did not find live node in hash table %u %p", x, ptr);
      assert(false);
    }
    if(nx->visited) {
      printf("CppGuard:Internal error: Did not clear visited marker on node %u\n", x);
      assert(false);
    }
    if(!ranks.insert(nx->rank)) {
      printf("CppGuard: Internal error: Duplicate occurrence of rank %d", nx->rank);
      assert(false);
    }
    HASH_FOR_EACH(y, nx->out)
    {
      ...
      if(nx->rank>=ny->rank) {
        printf("CppGuard Internal error: Edge %u->%d has bad rank assignment %d->%d\n", x, y, nx->rank, ny->rank);
        assert(false);
      }
    }
  }
...
TM_ATTRIBUTE_NOINLINE void GraphCycles::UpdateStackTrace(uint64_t id, SynchLocksHeld& p_all_locks) const
{
  ...
  n->nstack=GetStackFrames(n->stack, (int)mt_countof(n->stack), 2);
  n->priority=priority;
  n->thread_id=std::this_thread::get_id();
...
int GraphCycles::GetStackTrace(uint64_t id, void*** ptr) const
{
  Node* n=FindNode(rep_, id);
  if(n==nullptr) {
    *ptr=nullptr;
