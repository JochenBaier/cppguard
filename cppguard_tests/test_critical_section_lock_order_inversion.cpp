...
#include "../cppguard_tests_static_lib/test_in_c_file_shared_lib.h"
...
//FIXME some variables could be optimized away?
...
TEST_CASE("test_c_file_shared_lib_thread_sanitzer_example_exit_66")
{
  test_c_file_in_shared_lib_thread_sanitzer_example();
}

TEST_CASE("test_c_file_dyn_lib_thread_sanitzer_example_exit_66")
{
  run_c_test();
}

TEST_CASE("test_cpp_file_dyn_lib_thread_sanitzer_example_exit_66")
{
  run_cpp_test();
}
#endif
...
TEST_CASE("test_cs_too_many_locks_perh_thread_1000")
{
...
TEST_CASE("test_cs_statistis_wrong_init_delete_counter")
{
  std::vector<CRITICAL_SECTION> critical_sections;
  critical_sections.resize(1000);
  ...
  for(size_t i=0; i<critical_sections.size()-1; ++i)
  {
