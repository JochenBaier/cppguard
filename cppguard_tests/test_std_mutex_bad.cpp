...
TEST_CASE("test_std_mutex_100t_10_m_bulk_exit_66")
{
  ...
  for(size_t i=0; i<thread_count; ++i)
  {
    if(i==thread_count/2)
    {
      threads.emplace_back(test_bad_cs_100t_10cs_bulk_thread_function, critical_sections_wrong_order, std::ref(counter));
    } else
    {
      threads.emplace_back(test_bad_cs_100t_10cs_bulk_thread_function, critical_sections, std::ref(counter));
    }
  }
