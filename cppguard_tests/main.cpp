...
struct static_test_before_main_deadlock_t
{
  ...
  {
    if(std::getenv("CPPGUARD_STATIC_TEST_BEFORE_MAIN")==nullptr)
    {
      return;
    }
...
int main(int argc, char* argv[])
{
  ...
  if(std::getenv("CPPGUARD")==nullptr)
  {
    char error_text[1024]{};
    const int res=cppguard_set_options("thread_watchdog_max_duration_sec:5,halt_on_error:1", error_text, _countof(error_text));
    assert(res==1);
  }

  //context.addFilter("test-case", "*_exit_0");
