...
thread_watchdog_t::thread_watchdog_t(deadlock_monitor_t& p_deadlock_monitor, runtime_options_t& p_runtime_options) :
...
void thread_watchdog_t::add_thread(const std::thread::id& p_thread_id, SynchLocksHeldPtr& p_snch)
{
  ...
  const std::lock_guard<cs_mutex_t> lock(m_thread_list_mutex);
...
void  thread_watchdog_t::one_shot_timer_callback()
{
  ...
  {//scope
    ...
    for(auto& thread:m_threads)
    {
      ...
      const uint64_t max_duration_sec=(m_runtime_options.m_thread_watchdog_max_duration_sec<=120&&m_debugger_present) ? 120 : m_runtime_options.m_thread_watchdog_max_duration_sec.load();
  ...
  {//scope
    ...
    if(!m_threads.empty())
    {
      //printf("loop beendet startet neuen timer\n");
      m_one_shot_timer.start(m_runtime_options.m_thread_watchdog_max_duration_sec>=10 ? 5 : 1);
    }
  }
}
