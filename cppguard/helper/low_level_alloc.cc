...
bool LowLevelAlloc::DeleteArena(Arena* arena)
{
  ...
  while(arena->freelist.next[0]!=nullptr) {
    ...
    if((arena->flags&LowLevelAlloc::kAsyncSignalSafe)==0) {
      ...
      assert(false);
      //FIXME Linux munmap_result = DirectMunmap(region, size);
    }
#else
    munmap_result=munmap(region, size);
...
{
  if(v!=nullptr) {
    const AllocList* f=reinterpret_cast<AllocList*>(reinterpret_cast<char*>(v)-
      sizeof(f->header));
  ...
  if(request!=0) {
    ...
    for(;;) {  // loop until we find a suitable region
      ...
      if(new_pages==MAP_FAILED) {
        assert(false);
        //FIXME Linux ABSL_RAW_LOG(FATAL, "mmap error: %d", errno);
      }
